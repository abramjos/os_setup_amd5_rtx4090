...
#   8. DISPLAY_MANAGER: LightDM active+GDM masked (XFCE variants); GDM3 active (G/I)
#   9. DESKTOP_SESSION: XFCE running, no gnome-shell (XFCE variants); gnome-shell running (G/I)
#  10. NVIDIA_HEADLESS: (Variants C/G/H/I) Xorg NOT on NVIDIA GPU
...
#
# USAGE:
#   sudo bash verify-boot.sh [--variant A|B|C|...|I] [--auto] [--wait SECS]
...
###############################################################################
# Helper: detect which drm card is driven by amdgpu
###############################################################################
detect_amd_card() {
    for card in /sys/class/drm/card[0-9]; do
        local drv
        drv=$(basename "$(readlink "$card/device/driver" 2>/dev/null)" 2>/dev/null)
        if [ "$drv" = "amdgpu" ]; then
            basename "$card"
            return
        fi
    done
    echo "card0"  # safe fallback — amdgpu is almost always card0 on this hw
}
...
if [ -z "$DMUB_VER" ]; then
    ...
    # Compare full 32-bit version as integer. Minimum good version: 0x05002000 (0.0.32.0)
    # which eliminates the ring timeout cascade on Raphael (confirmed 2026-03-30).
    DMUB_HEX=$(echo "$DMUB_VER" | sed 's/0x//' | tr '[:lower:]' '[:upper:]')
    DMUB_INT=$(printf '%d' "0x${DMUB_HEX}" 2>/dev/null) || DMUB_INT=0
    MIN_GOOD=$(printf '%d' "0x05002000")

    if [ "$DMUB_INT" -ge "$MIN_GOOD" ]; then
        check_pass "DMUB_FIRMWARE" "$DMUB_VER (>= 0x05002000, ring-timeout fix confirmed)"
    else
        check_fail "DMUB_FIRMWARE" "$DMUB_VER (< 0x05002000 — outdated, run: sudo /usr/local/bin/update-dmcub-firmware.sh)"
    fi
fi
...
###############################################################################
DMUB_INIT=$(dmesg | grep -c "DMUB hardware initialized" 2>/dev/null) || DMUB_INIT=0
...
###############################################################################
OPTC31=$(dmesg | grep -c 'optc31_disable_crtc' 2>/dev/null) || OPTC31=0
if [ "$OPTC31" -eq 0 ]; then
...
###############################################################################
RING_TO=$(dmesg | grep -c 'ring gfx.*timeout' 2>/dev/null) || RING_TO=0
...
if [ "$VARIANT" = "G" ] || [ "$VARIANT" = "I" ]; then
    # GNOME variants use GDM3, not LightDM
    if [ "$GDM_STATUS" = "active" ]; then
        check_pass "DISPLAY_MANAGER" "GDM3 active (GNOME variant expected)"
    else
        check_fail "DISPLAY_MANAGER" "GDM3=$GDM_STATUS (expected active for GNOME variant)"
    fi
elif [ "$LIGHTDM_STATUS" = "active" ] && [ "$GDM_MASKED" = "masked" ]; then
...
###############################################################################
GNOME_RUNNING=$(pgrep gnome-shell 2>/dev/null | wc -l | tr -d ' ')
XFCE_RUNNING=$(( $(pgrep -c xfwm4 2>/dev/null) + $(pgrep -c xfce4-session 2>/dev/null) + 0 )) || XFCE_RUNNING=0

if [ "$VARIANT" = "G" ] || [ "$VARIANT" = "I" ]; then
    # GNOME variants: gnome-shell running is expected and correct
    if [ "$GNOME_RUNNING" -gt 0 ]; then
        check_pass "DESKTOP_SESSION" "gnome-shell running ($GNOME_RUNNING processes) — expected for GNOME variant"
    else
        check_fail "DESKTOP_SESSION" "gnome-shell not running (expected for GNOME variant — GDM/session failed?)"
    fi
elif [ "$GNOME_RUNNING" -eq 0 ] && [ "$XFCE_RUNNING" -gt 0 ]; then
...
###############################################################################
if [ "$VARIANT" = "C" ] || [ "$VARIANT" = "G" ] || [ "$VARIANT" = "H" ] || [ "$VARIANT" = "I" ]; then
    ...
    check_skip "NVIDIA_HEADLESS" "Not applicable for Variant $VARIANT (no NVIDIA GPU expected)"
fi
...
if [ "$VARIANT" = "C" ] || [ "$VARIANT" = "G" ] || [ "$VARIANT" = "H" ]; then
    # C/G/H: NVIDIA KMS enabled (modeset=1)
    for param in "nvidia-drm.modeset=1" "modprobe.blacklist=nouveau" "amdgpu.gfx_off=0" "amdgpu.dcdebugmask=0x10"; do
        if ! echo "$CMDLINE" | grep -q "$param"; then
            MISSING_PARAMS="$MISSING_PARAMS $param"
        fi
    done
elif [ "$VARIANT" = "I" ]; then
    # I: NVIDIA KMS disabled (modeset=0) — mutter sees only AMD iGPU
    for param in "nvidia-drm.modeset=0" "modprobe.blacklist=nouveau" "amdgpu.gfx_off=0" "amdgpu.dcdebugmask=0x10"; do
        if ! echo "$CMDLINE" | grep -q "$param"; then
            MISSING_PARAMS="$MISSING_PARAMS $param"
        fi
    done
...
###############################################################################
if [ "$VARIANT" = "G" ] || [ "$VARIANT" = "I" ]; then
    # GNOME variants: Mutter IS the compositor (by design).
    # Check that MUTTER_DEBUG_KMS_THREAD_TYPE=user is in the environment.
    MUTTER_KMS_TYPE=$(grep -r 'MUTTER_DEBUG_KMS_THREAD_TYPE' /etc/environment.d/ /etc/environment 2>/dev/null | grep -c 'user') || MUTTER_KMS_TYPE=0
    if [ "$MUTTER_KMS_TYPE" -gt 0 ]; then
        check_pass "COMPOSITOR" "Mutter compositor (GNOME) — KMS thread=user workaround confirmed"
    else
        check_warn "COMPOSITOR" "Mutter compositor (GNOME) — MUTTER_DEBUG_KMS_THREAD_TYPE=user not found in environment"
    fi
else
    # XFCE variants: xfwm4 compositor must be OFF
...
###############################################################################
AMD_CONNECTED=0
AMD_CARD_DISPLAY=$(detect_amd_card)
for conn in /sys/class/drm/${AMD_CARD_DISPLAY}-*/status; do
    [ -f "$conn" ] || continue
...
if [ "$OVERALL" = "FAIL" ]; then
    ...
    # Re-check DMUB version for recommendation (use same integer comparison as check 2)
    _DMUB_REC=$(dmesg | grep "Loading DMUB firmware" | head -1 | grep -oP 'version=0x[0-9a-fA-F]+' | sed 's/version=//')
    if [ -n "$_DMUB_REC" ]; then
        _DMUB_HEX=$(echo "$_DMUB_REC" | sed 's/0x//' | tr '[:lower:]' '[:upper:]')
        _DMUB_INT=$(printf '%d' "0x${_DMUB_HEX}" 2>/dev/null) || _DMUB_INT=0
        if [ "$_DMUB_INT" -lt "$(printf '%d' '0x05002000')" ]; then
